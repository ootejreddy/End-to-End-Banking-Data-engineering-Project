...
for message in consumer:
    ...
    # Debezium events are wrapped in a 'payload' envelope
    # The 'after' field contains the state of the row after the change
    payload = event.get("payload", {})
    record = payload.get("after")

    # If 'after' is null (e.g., delete events), we skip it for now
    if record:
        buffer[topic].append(record)
        print(f"[{topic}] -> {record}")  # Debugging

    # When buffer reaches batch_size, write to MinIO (S3) and clear buffer
    if len(buffer[topic]) >= batch_size:
        # topic name format: banking_server.public.customers -> customers
